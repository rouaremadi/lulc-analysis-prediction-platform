...
1. **Markov Chain Transition:** Calculates the probability of land cover transitioning from one class to another based on historical changes (1985 → 2025).
2. **Annual Matrix Derivation:** Uses eigenvalue decomposition to convert the 40-year transition matrix into an annual transition rate.
3. **Cellular Automata (CA) Smoothing:** Pure Markov chains ignore spatial context. To fix this, the standalone script applies a spatial filter where a polygon's predicted class is adjusted based on the majority class of its spatial neighbors, preventing "salt-and-pepper" artifacts.
...
- 📧 [rouaremadi5@gmail.com](mailto:rouaremadi5@gmail.com)
- 💼 [LinkedIn](https://linkedin.com/in/rouaremadi)
